...
public class LineArrowPathBuilder {
    ...
    public static Path getDirectLineArrowPath(float f, float f2, float f3, float f4, Arrow arrow, int i, float f5) {
        ...
        return buildArrowPath(f3 - (f6 * sqrt), f4 - (f7 * sqrt), f3, f4, ((float) arrowWidth) * f5, arrowLength, arrow.getType());
    }
    ...
    public static Path getQuadBezArrowPath(float f, float f2, float f3, float f4, float f5, float f6, Arrow arrow, int i, float f7) {
        ...
        while (true) {
            ...
            if (Math.abs(f11) <= 1.0f || f9 >= 1.0f || f9 <= 0.0f) {
                float f12 = quadBezComputePoint.x;
                float f13 = quadBezComputePoint.y;
            } else {
                if (f11 > 1.0f) {
                    f8 = f9 + f10;
                    if (bool != null && !bool.booleanValue()) {
                        f10 = (float) (((double) f10) * 0.1d);
                        f8 -= f10;
                    }
                    z = true;
                } else {
                ...
                Boolean bool2 = z;
                float f15 = f8;
                quadBezComputePoint = quadBezComputePoint(f, f2, f3, f4, f5, f6, f15);
                round = (int) Math.round(Math.sqrt((double) (((quadBezComputePoint.x - f5) * (quadBezComputePoint.x - f5)) + ((quadBezComputePoint.y - f6) * (quadBezComputePoint.y - f6)))));
                f10 = f10;
                bool = bool2;
                f9 = f15;
            }
        }
        float f122 = quadBezComputePoint.x;
        float f132 = quadBezComputePoint.y;
        return buildArrowPath(f122, f132, f5, f6, arrowWidth, arrowLength, arrow.getType());
    }
    ...
    public static Path getCubicBezArrowPath(float f, float f2, float f3, float f4, float f5, float f6, float f7, float f8, Arrow arrow, int i, float f9) {
        ...
        while (true) {
            ...
            if (Math.abs(i2) <= 1 || f11 >= 1.0f || f11 <= 0.0f) {
                float f13 = cubicBezComputePoint.x;
                float f14 = cubicBezComputePoint.y;
            } else {
                if (i2 > 1) {
                    f10 = f11 + f12;
                    if (bool != null && !bool.booleanValue()) {
                ...
                Boolean bool2 = z;
                float f16 = f10;
                cubicBezComputePoint = cubicBezComputePoint(f, f2, f3, f4, f5, f6, f7, f8, f16);
                round = (int) Math.round(Math.sqrt((double) (((cubicBezComputePoint.x - f7) * (cubicBezComputePoint.x - f7)) + ((cubicBezComputePoint.y - f8) * (cubicBezComputePoint.y - f8)))));
                f12 = f12;
                bool = bool2;
                f11 = f16;
            }
        }
        float f132 = cubicBezComputePoint.x;
        float f142 = cubicBezComputePoint.y;
        return buildArrowPath(f132, f142, f7, f8, (float) arrowWidth, (float) arrowLength, arrow.getType());
    }
