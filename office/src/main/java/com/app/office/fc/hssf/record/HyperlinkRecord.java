...
public final class HyperlinkRecord extends StandardRecord {
    ...
    private POILogger logger = POILogFactory.getLogger((Class) getClass());
    ...
    static final class GUID {
        ...
        private static long parseLELong(char[] cArr, int i) {
            ...
            for (int i2 = i + 14; i2 >= i; i2 -= 2) {
                j = (((j << 4) + ((long) parseHexChar(cArr[i2 + 0]))) << 4) + ((long) parseHexChar(cArr[i2 + 1]));
            }
            return j;
        }
    ...
    public HyperlinkRecord(RecordInputStream recordInputStream) {
        ...
        if (readInt == 2) {
            ...
            if (recordInputStream.remaining() > 0) {
                ...
                pOILogger.log(i3, (Object) "Hyperlink data remains: " + recordInputStream.remaining() + " : " + HexDump.toHex(recordInputStream.readRemainder()));
                return;
            }
            return;
        }
