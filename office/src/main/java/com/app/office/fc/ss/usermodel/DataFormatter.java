...
public class DataFormatter {
    ...
    public String formatRawCellContents(double d, int i, String str, boolean z) {
        ...
        return (!format3.contains(ExifInterface.LONGITUDE_EAST) || format3.contains("E-")) ? format3 : format3.replaceFirst(ExifInterface.LONGITUDE_EAST, "E+");
    }

    public String formatCellValue(ICell iCell) {
        return formatCellValue(iCell, (FormulaEvaluator) null);
    }
    ...
    public void setDefaultNumberFormat(Format format) {
        for (Map.Entry next : this.formats.entrySet()) {
            if (next.getValue() == this.generalDecimalNumFormat || next.getValue() == this.generalWholeNumFormat) {
    ...
    public static void setExcelStyleRoundingMode(DecimalFormat decimalFormat, RoundingMode roundingMode) {
        try {
            decimalFormat.getClass().getMethod("setRoundingMode", new Class[]{RoundingMode.class}).invoke(decimalFormat, new Object[]{roundingMode});
